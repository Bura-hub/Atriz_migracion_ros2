...
**🔴🔴 UNA PRUEBA MANUAL QUE HEREDA TU ENTORNO NO VERIFICA UN Exec* DE SYSTEMD.** El 2026-08-14
el vigía de DDS pasó su prueba manual detectando en 1,8 s… y en su primer arranque bajo systemd
declaró MUDO a un robot sano **dos veces** y lo reinició de más: el lanzador no cargaba
`/etc/profile.d/atriz-robot.sh`, así que escuchaba en el **dominio 0** con el robot en el 1 —
y la prueba manual no lo vio porque **mi shell ya traía `ROS_DOMAIN_ID=1` heredado**. La trampa
estaba escrita en la cabecera de `atriz-robot.sh` («systemd no lee /etc/profile.d») y se
repitió igual. Evidencia 113.
→ **La prueba fiel es `env -i HOME=… bash el-lanzador`** — con el entorno pelado, el arreglo
  detecta en 0,5 s y el fallo se habría visto ANTES de armar nada.
...
| **SLAM arrancado por el camino de la web** | `apagado → arrancando → funcionando` en **~18 s**, con los segundos subiendo (4 · 9 · 14). Y `CIEGO` en 1-2 s al apagar el barrido. ⚠️ **PRECISADO el 2026-08-15: «por el camino de la web» ≠ «desde el navegador».** Las dos evidencias que tocan esto —la **80** (botones SLAM) y la **111** (botón de Nav2)— se midieron **con guiones**, no con una pantalla: la 111 lo dice en su cabecera. No encuentro ninguna que enseñe un navegador haciéndolo. Y en este proyecto esa distinción no es cosmética: `ping`, `Resolve-DnsName` y `getent` dieron verde los tres con el navegador colgado | 2026-08-09 |
...
| ✅ ~~**PERO SIGUE BLOQUEANDO LA FASE 5: no hay identidad por usuario**~~ **CERRADO EL 2026-08-15 en rvr-01 (Fase B, evidencia 124) — y NO fue un proxy** | Lo que esta fila decía sigue siendo cierto de rosbridge 2.7.0 A SECAS (sin `rosauth`, sin `authenticate`, `check_origin()` incondicional): por eso la Fase B se resolvió **envolviendo el arranque** — `atriz_rvr_bringup/scripts/atriz_rosbridge.py` parchea `RosbridgeWebSocket.open` para exigir el **testigo Ed25519 en el subprotocolo** y ejecuta el nodo original. El diseño «proxy delante + rosbridge en 127.0.0.1» que anunciaba esta fila quedó **descartado** (metía un salto de Python en la ruta de 80,7 kB/s); `scripts/atriz_proxy.py` es código muerto y lo dice su cabecera. Verificado en producción por los dos lados: sin testigo desde la red → `4401` con motivo; `127.0.0.1` exento (las herramientas del robot siguen funcionando); y con `on_exit=Shutdown()` desde el mismo día, un puente muerto tumba la unidad en vez de dejar un robot verde y mudo. ⚠️ Lo que la Fase B **no** cierra: TLS (viaja en claro) y los otros 15 robots |
...
| 🔁 ~~**NO se reflashea rvr-01 para probar `provision.sh` entero**~~ — **SUPERADA EL 2026-08-10** | Decía «es el único robot montado» y sobre eso se **asumía** que `provision.sh` funciona. ✅ **CERRADA EL 2026-08-11: `provision.sh` se ejecutó ENTERO sobre rvr-02** (96 ✓ · 0 fallos) y el robot pasa el verificador (151 ✓ · 0 fallos), sin tocar rvr-01. 🔴 **Y aquí ponía una conclusión falsa que costó un día:** *«lo que sí está descartado es que la cause el guion — crea el directorio con `install -d -o "$USUARIO"` (`:244`)»*. **`:244` ERA el fallo**: `install -d` no aplica `-o`/`-g` a los padres que crea de paso, así que dejaba `~/atriz_ws` de **root** y `colcon build`, que corre como el usuario, moría con `Permission denied: 'log'`. Se descartó **leyendo el fuente** en vez de mirar el directorio — aplicada a un guion, «comprueba el efecto» significa que **mirar el código ES mirar el código de salida**. Evidencia 98. 📌 **Y la regla que valió más que el arreglo: lo que frene a rvr-02 va al GUION, no se arregla a mano** — los seis fallos de esa jornada acabaron en los guiones |
...
| ✅ **Al parar nav/slam, el barrido VUELVE AL ESTADO QUE LA UNIDAD ENCONTRÓ** (decisión B del usuario, 2026-08-14) | `atriz-escaneo on-recordando <unidad>` anota si `/scan` ya publicaba; `off-si-sobra [unidad]` consume la marca y lo deja encendido si estaba de antes — el mismo principio que `atriz.py` («apaga solo lo que encendiste»). ⚠️ No cubre a quien enciende *después* de arrancada la nav (queda el aviso de la web). Coste: +2 s de arranque de nav con el barrido apagado (~32 s). Evidencia 114 |
