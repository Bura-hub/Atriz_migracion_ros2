...
| **`provision.sh` NO instala systemd todavía** — se añade **cuando se cierre el robot de referencia** | mientras se prueba a mano en rvr-01, un servicio levantado pelearía por `/dev/rvr` con cada prueba. Decisión del usuario, 2026-07-31. ⚠️ **CORREGIDO 2026-08-01:** la imagen dorada **sí** llevaría el arranque automático (un `dd` copia todo). El problema es que **divergiría de `provision.sh`**, y la regla dice que gana el script. Evidencia 38 |
