...
| Medida | Por qué |
|---|---|
| 🔑 **La clave pública del testigo en `/etc/atriz/testigo.pub`** | 🔴 **Desde la Fase B (A7, 2026-08-15) sin ella el clon es INVISIBLE PARA LA WEB.** `atriz_rosbridge.py` falla cerrado, así que el robot arranca, parece sano y no hay forma de hablarle desde la interfaz — y no se nota hasta que alguien lo intenta. Va **dentro de la imagen** porque los 16 comparten el mismo servidor web, o sea la misma clave. `fase_6` **aborta** si falta, y `verificar_robot.sh` da FALLO. Se emite con `node herramientas/publicar_clave.mjs` en `atriz-lab` |
