...
def simular_girar(grados_pedidos, yaw_generador, freq_hz=20.0):
    ...
    Args:
        ...
        yaw_generador: función que devuelve yaw en radianes dado (iteración, restante_grados)
    ...
    anterior = yaw_generador(0, grados_pedidos)
    acumulado = 0.0
...
if __name__ == '__main__':
    ...
    def yaw_ideal(iteracion, restante_grados):
        ...
        if iteracion == 0:
            return 0.0
        # Simular velocidad angular según la rampa de velocidad_giro()
        # Simplificado: velocidad promedio ~0.5 rad/s
        return iteracion * (1.0 / 20.0) * 0.5

    def yaw_congelado_en_45(iteracion, restante_grados):
        """/odom se congela a los 45.8°."""
        if iteracion >= 50:
            return math.radians(45.8)
        return iteracion * (1.0 / 20.0) * 0.5

    def yaw_con_duplicados_ocasionales(iteracion, restante_grados):
