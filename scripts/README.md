...
| `provision.sh` | B–**F** | sí | ✅ **EJECUTADO ENTERO el 2026-08-11 en rvr-02**, sobre un 24.04 limpio: **96 ✓ · 16 avisos · 0 fallos**. No a la primera — tiró los dos últimos pasos por un `install -d` que dejaba `~/atriz_ws` de root (evidencia 98). 📝 Desde el 2026-08-01 instala también el **arranque automático** (paso 8/9). 📝 Desde el 2026-07-31 deja el robot **completo** (8 pasos, incluida la Etapa F) y clona la rama **`ros2`** — antes clonaba `migracion-ros2`, que es código de ROS 1 y no compila |
...
| `sistema/atriz-lidar-reenganche.*` (+ `98-…rules`) | operación | sí (instalarlos) | ✅ **verificado desenchufando de verdad el 2026-08-14** (evidencia 115): udev dispara la oneshot al reaparecer el adaptador y reinicia atriz-robot SOLO con el descriptor `(deleted)` — ~22 s a robot útil. Cinco guardias, anti-aleteo 120 s. 🔴 Y corrigió una atribución: lo que re-enumera es **desenchufar el USB de la Pi**, no apagar el RVR |
| `atriz-vigia-dds.sh` + `sistema/vigia_dds.py` | operación | sí (instalarlos) | ✅ **instalado y verificado en sus tres ramas el 2026-08-14** (evidencia 113): el robot mudo en DDS (evidencia 109) se cura solo — UNA vez por arranque, fallo abierto. 🔴 Sus dos primeros disparos fueron falsos positivos por `ROS_DOMAIN_ID` ausente: la prueba fiel de un ExecStartPost es con `env -i`, no heredando tu shell |
