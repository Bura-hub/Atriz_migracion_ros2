...
| ~~B2~~ | ✅ **CERRADO el 2026-08-07.** Arranca (`active/success`) con el mapa del cuarto, y tarda **entre 18 y 26 s** hasta aceptar objetivos. `TimeoutStartSec=120` **cabe**. Evidencia 79 | **MEDIDO** | 🔴 Y sale una consecuencia que decide el diseño: `systemctl start` **bloqueó 26,1 s** contra los **5,0 s** de los tres plazos de la cadena web. **`--no-block` es obligatorio**, ya no como precaución sino con 5× de margen medido |
| ~~B3~~ | ✅ **CONFIRMADO el 2026-08-07, y es PEOR: el botón es de UNA pulsación.** `StartLimitBurst` cuenta *arranques*, no clics — el inicial más dos reintentos automáticos son ya los tres, en ~40 s. La unidad queda `failed` y solo sale con `reset-failed`, o sea con privilegio. Evidencia 79 | **MEDIDO** | El servicio ROS **tiene que negarse antes** de llamar a systemctl si no hay mapa. Y `Restart=no` merece considerarse para una unidad que se pide a mano |
| ~~B4~~ | ✅ **CERRADO el 2026-08-07.** `slam.launch.py` tiene ya el guardia simétrico: se niega si hay un `async_slam_tool` **o** un `amcl` vivos. `Atriz_rvr@fac74bf` | **VERIFICADO en las tres direcciones** con procesos falsos, sin arrancar SLAM: limpio → pasa · slam vivo → se niega · amcl vivo → se niega | Queda abierta la **carrera**: `ps` mira un instante y dos clientes simultáneos pueden pasar los dos. Necesita un cerrojo en quien orqueste las unidades, no en el launch |
