...
| ✅ **atriz-nav bajo systemd y el material docente en el suelo** | **2026-08-13, en el laboratorio** (evidencias 104-108): la navegación entera arranca con `systemctl start atriz-nav` y acepta objetivos en **27,8 s** (n=2, Δ 0,04 s); M10 medido (`PartOf=`+`Requires=` **vuelve** tras `kill -9`); A11 cerrado (la capa de seguridad **no** está inerte); y la sesión física docente **en banda** con el usuario midiendo — solo queda la práctica 63 (sin línea aún). 🔴 Confirmado también: un `start` sin mapa deja el botón de la web **muerto** (latch de systemd) — ✅ y el guardia que protege a la web **ya existía** (`supervisor_navegacion` se niega antes de systemctl desde el 2026-08-07, rechazos verificados en la evidencia 80; el «sin implementar» que estuvo aquí se retiró el 2026-08-14: era un negativo escrito sin mirar el código). ⏳ El mapa del aula quedó pendiente ese día — ✅ **hecho el 2026-08-19**, ver la fila de abajo |
| ✅ **El robot se cura solo de dos fallos** | **2026-08-14** (evidencias 109-115): el **vigía de DDS** reinicia el stack si nace mudo (una vez por arranque, fallo abierto — el mudo reapareció con las esperas cumplidas y su causa sigue sin conocerse), y el **reenganche del LIDAR** por udev recupera al robot en **~22 s** si alguien desenchufa el USB del X2 (verificado desenchufando de verdad). Además: el botón de Nav2 funcionó de extremo a extremo (28,4 s, n=3), el **latch se limpia solo a los ~5 min**, el barrido **vuelve al estado que la unidad encontró** al parar la nav (decisión B), y el caudal de `/estado_robot` quedó medido (0,35 kB/s — el 0,03 que circulaba era copiado) |
| ✅ **El Taller (terminal del alumno) vive en rvr-01** | **2026-08-15** (evidencia 117): la mitad-robot del terminal que construyó el PC quedó auditada y validada EN VIVO — cinco fallos cazados con experimento (dos en vivo), 19 casillas en verde, y la **práctica 05 corriendo de punta a punta por el agente** con el sensor real. `atriz-agente` instalado y habilitado por `fase_7`, con clave de prueba; al PC le quedan la clave real y el navegador |
| ✅ **LA ARENA MAPEADA, A5 CERRADO Y NAV2 MEDIDO EN EL SITIO** | **2026-08-18/19, en el laboratorio.** `~/mapas/arena.yaml` — arena **cuadrada de ~3,95 × 4,00 m**, muros alineados con los ejes y **origen anclado a una esquina convenida**, que es lo que hace el mapa utilizable: sin morro paralelo a la pared el mapa sale **en rombo**, y una sala cuadrada **no se puede localizar** casando `/scan` (tres candidatos empatados). ✅ **A5 cerrado**: la pose que fija `/initialpose` es correcta — cinta contra AMCL, **n=2: 2,7 y 6,9 cm**, banda ≤10 declarada antes de medir. ✅ **AMCL con objetos: sin degradación medible** (0,6 cm; los objetos eran el **2,6 % del barrido**). 🔴 **Nav2 llega a ~15 cm y dice `SUCCEEDED` las dos veces**: la tolerancia de 10 cm **no se cumple**, porque el desenlace hereda el error de AMCL. ✅ **Y el mismo día cerraron los dos que quedaban:** la configuración **dura** de objetos (18,6 % del barrido ocupado) **tampoco degrada** a AMCL (1,2 cm), y los **huecos** quedaron acotados sin mover el robot — con el mapa SIN los objetos, **40,3 cm → 1 celda transitable · 45,0 → 2 · 48,1 → 2**. ✅ **El 2026-08-20 se cerró el BLOQUE C entero:** la **travesía** (por 40,1 cm el robot pasa **reptando** — 41 % de lo mandado dentro de la puerta y 11 % al salir; de ahí la regla **`hueco practicable ≈ 2 × radius + 2 × deriva`**, o sea 40 cm el límite y 45-50 lo cómodo) y el **caso engordado** (remapeando **con** los objetos, ese mismo hueco da **0 celdas, 49/49 cerradas**: el engorde de la capa estática es la diferencia entre pasar y no pasar) |
| **Siguiente paso** | 🔴 **ANTES DE `fase_6`, UNA DECISIÓN NUEVA (2026-08-19): cómo llega el mapa de la arena a los 16.** El del laboratorio ya existe (`~/mapas/arena.yaml`) y vive **fuera** del paquete, así que `fase_6` **lo borraría** y no hay mecanismo que se lo lleve a los otros 15 — y aquí clonarlo **sí** es correcto, porque los 16 comparten la misma arena. Las dos salidas y sus costes, en `00_auditoria/planes/2026-08-03-arranque-navegacion.md`. · **`fase_6_preparar_imagen_dorada.sh`** no tiene ningún bloqueante **técnico**: los tres auditados el 2026-08-01 están cerrados. 📌 El aviso de `ATRIZ_MAPA` que sale en rvr-02 **no lo bloquea** — `fase_6` borra los mapas y vacía esa variable **a propósito**, porque repartir el mapa del robot de referencia haría que Nav2 dijera «llegué» estando a medio metro en los otros 15 (evidencia 84). Cada aula mapea el suyo. Y después, **la plataforma web (Fase 5)**, que es lo único grande que queda |
...
├── atriz-vigia-dds.sh + sistema/vigia_dds.py   ✅ el robot mudo en DDS se cura solo (ev. 113)
├── sistema/atriz-lidar-reenganche.*   ✅ el LIDAR desenchufado se cura solo, por udev (ev. 115)
└── first-boot.sh / .service  📝 NO VERIFICADO — personaliza cada robot clonado
```

`evidencia_24_04/`, `evidencia/mediciones_banco/` y `scripts/` tienen su propio `README.md` con
el detalle. (`evidencia/` no lleva índice: son ficheros numerados y el `CHANGELOG.md` hace de
índice cronológico.)
